...
class Upcoming extends MY_Controller {
	...
	private $name = 'upcoming';
    private $title = 'upcoming events';
    private $table = "upcoming";
    protected $redirect = "upcoming";
	...
	public function get()
    {
        $fetch_data = $this->main->make_datatables(admin('upcoming_model'));
        $sr = $_POST['start'] + 1;
        $data = array();
        ...
        {  
            $sub_array = array();
            $sub_array[] = $sr;
            $sub_array[] = ucwords($row->event);
            $sub_array[] = date('d-m-Y', strtotime($row->event_date));
            $sub_array[] = img(['src' => 'assets/images/event/'.$row->image, 'height' => 50, 'width' => 50]);
        ...
        $output = array(  
            ...
            "recordsTotal"      => $this->main->count($this->table, ['is_deleted' => 0, 'event_date >= ' => date('Y-m-d')]),
            "recordsFiltered"   => $this->main->get_filtered_data(admin('upcoming_model')),
            "data"              => $data,
            $csrf_name          => $csrf_hash
        );
	...
	public function view($id)
    {
        ...
        $data['operation'] = "view";
        $data['url'] = $this->redirect;
        $data['data'] = $this->main->get($this->table, 'event, event_date, image', ['id' => d_id($id)]);
	...
	public function edit($id)
	{
        ...
        $data['inputmask'] = TRUE;
        $data['url'] = $this->redirect;
        $data['data'] = $this->main->get($this->table, 'id, event, event_date, image', ['id' => d_id($id)]);
	...
	public function showImages($id)
	{
        ...
        if ($images) {
            ...
            foreach ($images as $k => $v) {
                $image[$k]['url'] = assets('images/event/');
                $image[$k]['image'] = $v;
            }
        }else
            $image = '';
